(() => {
  ...
  var FetchRequest = class {
    ...
    async perform() {
      ...
      try {
        ...
        const response = await fetch2(this.url.href, fetchOptions);
  ...
  var Preloader = class {
    ...
    async preloadURL(link) {
      ...
      try {
        const response = await fetch2(location2.toString(), { headers: { "Sec-Purpose": "prefetch", Accept: "text/html" } });
...
%o`, message, error2, detail);
  ...
  function initSimpleMap() {
    ...
    const { Map: Map2, Marker } = google.maps;
